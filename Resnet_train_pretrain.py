...
parser.add_argument('--pretrained_weights', default='models/resnet18_0.7412_pretrain无生成数据已确认无法继续优化.pth', type=str)
...
if __name__ == '__main__':
    ...
    train_dataset = fair_vl_med_dataset(args.dataset_dir, transform_train,use_gen_data=args.use_gen_data, subset='Training', text_source=args.text_source, summarized_note_file=args.summarized_note_file,files=train_files)
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,
        num_workers=args.workers, pin_memory=True, drop_last=False)

    val_dataset = fair_vl_med_dataset(args.dataset_dir, transform_test, subset='Validation')
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False,
        num_workers=args.workers, pin_memory=True, drop_last=False)

    test_dataset = fair_vl_med_dataset(args.dataset_dir, transform_test, subset='Test')
    ...
    if args.use_gen_data:
        output_name = f'resnet18使用生成数据-seed{args.seed}_auc{best_auc:.4f}'
    else:
        output_name = f'resnet18不使用生成数据-seed{args.seed}_auc{best_auc:.4f}'
    print(output_name)
